<h1 align="center">SP-IE-SRGS v0</h1>

This repository is a local research fork for Source-Routed IE-SRGS. It starts
from the reproduced `mip-splatting/hybrid_sdfgs` IE/SR prior pipeline and adds
the v0 scaffolding for:

- source-aware parameter routing;
- geometry-only densification statistics;
- target-resolution geometry rendering with LR downsample consistency;
- ramped SOF surface regularization;
- routing and densification gradient audit logs.

The routed training loop is not integrated yet. The current implementation
boundary is documented in `docs/implementation_plan.md`, with reusable code in
`routing/` and `surface/`.
